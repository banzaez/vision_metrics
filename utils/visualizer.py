...
class Visualizer:
    ...
    def draw(self, frame, detections, roi=None, staff_auto_zones=None, groups=None):
        ...
        :return: Кадр с наложенной графикой или None, если кадр пустой
        """
        if frame is None:
            return None
        ...
        if self._draw_options["staff_zones"] and staff_auto_zones:
            for sz in staff_auto_zones:
                sx1, sy1, sx2, sy2 = sz
                cv2.rectangle(
                    frame, (sx1, sy1), (sx2, sy2), self.colors["staff_auto"], thickness
                )
                cv2.putText(
                    frame,
                    "STAFF ZONE",
                    (sx1, sy1 - int(10 * scale)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    font_scale * 0.8,
                    self.colors["staff_auto"],
                    max(1, thickness - 1),
                )
