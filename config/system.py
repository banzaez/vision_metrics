...
class PerformanceConfig:
    ...
    # ПРИМЕЧАНИЕ: Это значение автоматически переопределяется реальным FPS при открытии видео.
    # Номинальный FPS видео. Используется как fallback, если метаданные недоступны.
    frame_rate: int = 25
